...
@Component
public class FileUtilsForOrder {//쇼핑몰 이미지 정보 저장공간/ 데이터베이스랑 불러올 dto도 만듬, 이제 mybatis만 만들어서 따로 경로 설정하면 됨.=> order controller 다시 수정

	/**
	 * 파일 업로드 정보를 파싱하여 BoardFileDto 객체 리스트로 반환하는 메서드.
	 * @param orderIdx 게시물 인덱스
	 * @param multipartHttpServletRequest 파일이 포함된 HTTP 요청
	 * @return 업로드된 파일 정보 리스트
	 * @throws Exception 파일 처리 중 발생할 수 있는 예외
	 */
	public List<OrderFileDto> parseFileInfo(int orderIdx, MultipartHttpServletRequest multipartHttpServletRequest) throws Exception {
		...
		List<OrderFileDto> fileList = new ArrayList<>();
		...
		String path = "./src/main/resources/static/orders/" + current.format(format);
		...
		while (iterator.hasNext()) {
			...
			for (MultipartFile multipartFile : list) {
				...
				if (multipartFile.isEmpty() == false) {
					...
					OrderFileDto orderFile = new OrderFileDto(); // DTO 객체 생성

					// DTO에 파일 정보 설정
					orderFile.setOrderIdx(orderIdx);
					orderFile.setFileSize(multipartFile.getSize());
					orderFile.setOriginalFileName(multipartFile.getOriginalFilename());

					// 저장 경로 설정 (상대 경로로 설정)
					orderFile.setStoredFilePath("/orders/" + current.format(format) + "/" + newFileName);
