...
public class BoardController {
    ...
    public String openBoardList(@RequestParam(value = "type" , required = false , defaultValue = "all") String type, @RequestParam(value = "keyword",required = false , defaultValue = "no") String keyword, Model model,@PageableDefault(page = 0, size = 10) Pageable pageable) throws Exception {
        String key = keyword.trim();
        List<BoardDto> list = boardService.configuration(type,key);
...
//		model.addAttribute("list", list);
    ...
    public String insertBoard(@RequestParam(value = "type") String type,BoardDto board, MultipartHttpServletRequest multipartHttpServletRequest, HttpSession session) throws Exception {
        AuthInfo auth = (AuthInfo) session.getAttribute("authInfo");
        board.setCreatorId(auth.getNick());
        board.setType(type);
...
//	}
    ...
    @RequestMapping("/board/openBoardDetail.do")
    public String openBoardDetail(@RequestParam int boardIdx,@ModelAttribute("flag") String flag, Model model, HttpSession session) throws Exception {
        if(!flag.equals("true")){
